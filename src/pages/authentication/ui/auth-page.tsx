...
export function AuthPage({
  ...
  async function onSubmitCustomLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      persistSession(accessToken, email.trim(), 'custom');
  ...
  async function onSubmitAsoLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      persistSession(accessToken, email.trim(), 'aso');
  ...
  async function loginAsAsoTestUser() {
    ...
    try {
      ...
      persistSession(accessToken, TEST_USER.email, 'aso');
      void navigate(resolveAsoPath(redirectTo), { replace: true });
    } catch (err) {
      setError(err instanceof Error ? err.message : t('auth.errorGeneric'));
    } finally {
            ...
            {isLogin ? (
              <Tabs value={loginProvider} onValueChange={selectLoginProvider}>
                ...
                <TabsContent value="aso" className="space-y-4">
                  ...
                  <button
                    type="button"
                    className="text-foreground h-12 w-full cursor-pointer rounded-md border border-dashed bg-transparent px-4 text-sm font-medium underline-offset-4 hover:underline disabled:cursor-not-allowed disabled:opacity-50"
                    disabled={pending}
